...
vector<string> Graph::Dijkstra(string city, string destination, float &d)
{
	...
	pqueue<node>* hp = new pqueue<node>(arcs.size()+10);					//priority queue of nodes
	...
	while(!found)
	{
		...
		node minn = hp->extractMin();
		node* min = node_list[minn.name];  //finds the pointer to that node in the map
