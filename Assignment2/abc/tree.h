...
public:
	...
	TreeItem *getpointer(); // returns the root of the tree


	int countFiles(); // returns the total count of the files in the system

	vector<string> toVector(string input); //returns the file path in vector form

    void insertInTree(vector<string> result, TreeItem* current, TreeItem* parent, int i); //function to insert nodes in tree

    TreeItem* findInTree(string toFind, TreeItem* current);
